...
export default function CrossCalendarPage() {
  ...
  const handleGenerate = async () => {
    if (!selectedEpreuve) {
      toast("Choisissez une \u00e9preuve", "error");
      return;
    }
    setGenerating(true);
    try {
      ...
      toast(
        `${res.data.summary.totalRooms} cr\u00e9neaux g\u00e9n\u00e9r\u00e9s !`,
        "success",
      );
    } catch (e: any) {
      toast(
        e.response?.data?.error || "Erreur de g\u00e9n\u00e9ration",
        "error",
      );
    } finally {
      setGenerating(false);
  ...
  const memberMap = useMemo(() => {
    const map: Record<
      string,
      { email: string; color: (typeof MEMBER_COLORS)[0] }
    > = {};
    let colorIdx = 0;
  ...
  const maxOverlap = useMemo(() => {
    ...
    Object.values(slotMap).forEach((v) => {
      if (v.members.size > max) max = v.members.size;
    });
    return max;
  }, [slotMap]);
      ...
      <div className="flex flex-wrap gap-2 shrink-0">
        ...
        {memberList.length === 0 && (
          <div className="text-sm text-gray-400 flex items-center gap-2">
            <Users size={16} />
            Aucune disponibilit&eacute; cette semaine
          </div>
        )}
      </div>
        ...
        <CardContent className="flex-1 p-0 overflow-auto flex">
          {days.map((day) => {
            ...
            const dayAvails = availabilities.filter(
              (a) =>
                a.date && format(new Date(a.date), "yyyy-MM-dd") === dateStr,
            );

            return (
              <div
                key={dateStr}
                className="flex-1 min-w-[180px] border-r border-gray-100 last:border-r-0 flex flex-col"
              >
                ...
                <div
                  className={cn(
                    "relative flex-1",
                    dayConfig.isOpen ? "bg-white" : "bg-gray-100",
                  )}
                ...
                >
                    ...
                    Array.from({ length: numberOfSlots }).map((_, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={cn(
                            "absolute w-full border-gray-100 text-xs pl-1 pointer-events-none flex items-center",
                            isFullHour
                              ? "border-b-2"
                        ...
                        >
                          {isFullHour && (
                            <span className="-mt-[20px] text-gray-300">
                              {h}:00
                            </span>
                          )}
                          {overlapCount >= 2 && (
                            <span
                              className={cn(
                                "absolute right-1 top-0.5 px-1.5 py-0.5 text-white text-[10px] font-bold rounded z-30",
                                overlapCount >= membersPerSlot
                                  ? "bg-emerald-500"
                                  : "bg-orange-400",
                              )}
                            >
                  ...
                  {dayConfig.isOpen &&
                    dayAvails.map((avail: any, idx: number) => {
                      ...
                      const membersAtSlot = slotData
                        ? Array.from(slotData.members)
                        : [avail.memberId];
                      ...
                      return (
                        <div
                          key={avail.id || idx}
                          className={cn(
                            "absolute rounded border-l-3 px-1 py-0.5 text-[10px] overflow-hidden z-20 border",
                            memberColor.bg,
                            memberColor.border,
                            memberColor.text,
                          )}
                          ...
                          title={`${memberMap[avail.memberId]?.email} - ${avail.startTime} to ${avail.endTime}`}
                        >
                          <div className="truncate font-medium">
                            {memberMap[avail.memberId]?.email.split("@")[0]}
                          </div>
                        </div>
                      );
                    })}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">
              Param&egrave;tres de g&eacute;n&eacute;ration
            </CardTitle>
            <CardDescription>
              Croisez les disponibilit&eacute;s pour cr&eacute;er des
              cr&eacute;neaux.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between bg-gray-50 p-2 rounded">
              ...
              <span className="text-sm font-medium">
                Sem. du {format(currentWeekStart, "d MMM", { locale: fr })}
              </span>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div>
              ...
              <Input
                ...
                value={maxCandidates}
                onChange={(e) =>
                  setMaxCandidates(parseInt(e.target.value) || 1)
                }
              />
              <p className="text-xs text-gray-500 mt-1">
                1 = individuel, 2-6 = groupe
              </p>
            </div>
            ...
            <Button
              ...
              disabled={generating || !selectedEpreuve}
            >
              {generating ? (
          ...
          <div className="md:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {generatedResult.summary.totalRooms} cr&eacute;neaux &middot;
                  Capacit&eacute; totale:{" "}
                  {generatedResult.summary.totalCapacity} candidats
                </CardDescription>
              </CardHeader>
              <CardContent>
                        ...
                        {slot.rooms.map((room: any) => (
                          ...
                          >
                            ...
                            <span className="text-gray-500 ml-2">
                              ({room.maxCandidates} candidats max)
                            </span>
                            <div className="mt-1 text-gray-600">
                              {room.members
                                .map((m: any) => m.email.split("@")[0])
                                .join(", ")}
                            </div>
                          </div>
                        ))}
  ...
  const renderSlots = () => {
    ...
    const publishedSlots = existingSlots.filter(
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-500">
                Aucun cr&eacute;neau cr&eacute;&eacute;
              </h3>
              <p className="text-sm text-gray-400 mt-2">
                Utilisez l&apos;onglet &quot;G&eacute;n&eacute;rer&quot; pour
                cr&eacute;er des cr&eacute;neaux.
              </p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={cn(
                          ...
                          statusColors[slot.status] || "bg-gray-100",
                        )}
                      >
                      ...
                      <div className="flex gap-1 justify-end">
                        {slot.status === "draft" && (
                          <Button
                            ...
                            onClick={() =>
                              handleBulkStatus([slot.id], "published")
                            }
                          >
                            <Unlock size={14} />
                          </Button>
                        ...
                        {slot.status === "published" && (
                          <Button
                            ...
                            onClick={() =>
                              handleBulkStatus([slot.id], "closed")
                            }
                          >
                            <Lock size={14} />
                          </Button>
  ...
  if (loading && activeTab === "availabilities")
    return (
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-2xl font-semibold">
            Calendriers Crois&eacute;s &amp; Cr&eacute;neaux
          </h1>
          <p className="text-sm text-gray-500">
            G&eacute;rez les disponibilit&eacute;s, g&eacute;n&eacute;rez des
            cr&eacute;neaux et g&eacute;rez les inscriptions.
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 shrink-0 w-fit">
        {[
          ...
          {
            key: "generate" as TabType,
            label: "G\u00e9n\u00e9rer cr\u00e9neaux",
            icon: Calendar,
          },
          {
            key: "slots" as TabType,
