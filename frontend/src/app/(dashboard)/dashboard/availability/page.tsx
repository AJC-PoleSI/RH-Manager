...
export default function AvailabilityPage() {
  ...
  const fetchAvailabilities = useCallback(async () => {
    ...
    try {
      ...
      const data = res.data.map((s: any, i: number) => ({
        ...s,
        id: s.id || `server-${i}`,
      }));
      setSlots(data);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(e);
      const msg =
        e.response?.data?.error || e.message || "Erreur lors de la sauvegarde";
      toast(msg, "error");
    } finally {
      setSaving(false);
    }
  ...
  const toggleSlot = (date: Date, startTime: string, endTime: string) => {
    ...
    const exists = slots.find((s) => {
      const slotDateStr = s.date
        ? format(new Date(s.date), "yyyy-MM-dd")
        : null;
      if (slotDateStr)
        return slotDateStr === dateStr && s.startTime === startTime;
      if (s.weekday)
        return parseInt(s.weekday) === weekday && s.startTime === startTime;
      return false;
    });
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-semibold text-gray-900 flex items-center gap-3">
            Mes Disponibilités
            {!saisieOuverte && (
              <span className="text-xs px-3 py-1 bg-red-100 text-red-700 rounded-full font-bold uppercase tracking-wide">
                Lecture Seule
              </span>
            )}
          </h1>
          <p className="text-gray-500 mt-2 font-medium">
            Cochez simplement les créneaux où vous êtes disponible.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-xl border border-gray-100">
          ...
          <span className="text-sm font-bold text-gray-700 w-36 text-center">
            ...
            {format(addDays(currentWeekStart, 4), "d MMM", { locale: fr })}
          </span>
          <Button
            variant="ghost"
            className="hover:bg-white"
                    ...
                    daySlots.map((slot, i) => {
                      ...
                      return (
                        ...
                        >
                          {slot.start} - {slot.end}
                          {selected && (
                            <CheckCircle2 className="absolute right-4 w-5 h-5 text-purple-500" />
                          )}
                        </button>
                      );
                    })
      ...
      {saisieOuverte && (
        <div className="shrink-0 bg-white border border-gray-100 p-4 rounded-2xl shadow-lg flex justify-between items-center sticky bottom-6 z-50">
          <p className="text-sm font-semibold text-gray-500 px-4">
            N&apos;oubliez pas d&apos;enregistrer vos modifications une fois
            terminé.
          </p>
          <Button
            size="lg"
