...
export default function MessagesPage() {
    ...
    const fetchContacts = useCallback(async () => {
        try {
            ...
            const candidatContacts: Contact[] = (candidatesRes.data?.data || []).map((c: any) => ({
                id: c.id,
                name: `${c.firstName || c.first_name || ''} ${c.lastName || c.last_name || ''}`.trim(),
                type: 'candidat' as const,
            }));

            const membreContacts: Contact[] = (membersRes.data || []).map((m: any) => ({
                id: m.id,
    ...
    const currentMessages = selectedContact
        ? allMessages.filter(
            m => (m.senderId === selectedContact.id || m.recipientId === selectedContact.id)
        )
        : [];
    ...
    const handleSend = async () => {
        ...
        try {
            await api.post('/messages', {
                recipientId: selectedContact.id,
                recipientRole: selectedContact.type === 'candidat' ? 'candidate' : 'member',
                message: messageText,
                senderName: user?.email?.split('@')[0] || 'Moi',
            });
        } catch (e) {
        ...
        return (
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Envoyez des messages aux candidats et membres</p>
                </div>
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Messages re&ccedil;us de l&apos;administration</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col" style={{ height: '600px' }}>
                ...
                <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
                    <p className="text-xs text-gray-400 text-center">Les messages sont envoy&eacute;s par l&apos;administration. Vous ne pouvez pas r&eacute;pondre.</p>
                </div>
            </div>
        </div>
    );
