...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const slotMap: Record<string, { members: { id: string; email: string }[]; weekday: string; startTime: string; endTime: string }> = {};
    ...
    for (const [, slotData] of validSlots) {
      ...
      const sortedMembers = [...slotData.members].sort((a, b) => {
        return (memberAssignmentCount[a.id] || 0) - (memberAssignmentCount[b.id] || 0);
      });
      ...
      for (let room = 0; room < actualRooms; room++) {
        ...
        for (let m = 0; m < membersPerRoom && memberIndex < sortedMembers.length; m++, memberIndex++) {
          ...
          if (!assignError) {
            ...
            memberAssignmentCount[member.id] = (memberAssignmentCount[member.id] || 0) + 1;
          }
        }
    ...
    return Response.json({
      ...
      summary: {
        ...
        totalAssignments: createdAssignments.reduce((sum, a) => sum + a.members.length, 0),
