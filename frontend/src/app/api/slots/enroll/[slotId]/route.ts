...
export async function DELETE(
  ...
  try {
    ...
    const now = new Date();
    const hoursUntilStart = (slotStartDateTime.getTime() - now.getTime()) / (1000 * 60 * 60);

    if (hoursUntilStart < 24) {
      return Response.json(
        { error: 'Annulation impossible : le créneau commence dans moins de 24 heures.' },
        { status: 403 }
      );
    }
    // ══════════════════════════════════════════════════════════════════
    ...
    if (updatedSlot && updatedSlot.status === 'full') {
      const newStatus =
        (updatedSlot.members?.length || 0) >= updatedSlot.min_members ? 'ready' : 'open';
      await supabaseAdmin
        .from('evaluation_slots')
