...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    results.push(err2 ? `documents_urls: ${err2.message}` : 'documents_urls: OK');

    return Response.json({
      success: true,
      message: 'Migration attempted. If rpc exec_sql is not available, add columns manually in Supabase Dashboard.',
      results,
    });
  } catch (error) {
