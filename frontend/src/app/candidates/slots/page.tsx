...
export default function CandidateSlotsPage() {
  ...
  useEffect(() => {
    fetchData();
    // `startPolling` gère déjà le retour sur l'onglet (refresh immédiat).
  ...
  const slotsByEpreuve = slots.reduce(
  ...
  if (planningVisible === false) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-semibold">Inscription aux créneaux</h1>
          <p className="text-sm text-gray-500 mt-1">
            Choisissez vos créneaux d&apos;évaluation parmi ceux disponibles.
          </p>
        </div>
        <div className="flex items-center justify-center py-16">
          <div className="text-center max-w-md">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              Les créneaux d&apos;évaluation ne sont pas encore disponibles.
              Vous serez informé(e) dès qu&apos;ils seront publiés.
            </p>
            ...
            <div className="grid gap-3">
              {enrollments.map((e: any) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg flex-wrap gap-2"
                  >
                    <div className="flex items-center gap-4 flex-wrap">
                      <div className="flex items-center gap-2 text-green-700">
                        <Calendar size={16} />
                        <span className="font-medium">
                          {format(new Date(e.date), "EEEE d MMMM", {
                            locale: fr,
                          })}
                        </span>
                      </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-gray-400 mt-2">
              Les créneaux d&apos;inscription ne sont pas encore ouverts.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          <Card key={epreuveName}>
            <CardHeader>
              ...
              <p className="text-sm text-gray-500">
                Tour {(epreuveSlots as any[])[0]?.tour} &middot;{" "}
                {(epreuveSlots as any[]).filter((s: any) => !s.isFull).length}{" "}
