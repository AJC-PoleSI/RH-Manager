...
// Trombinoscope des candidats — vue « photos » de la page Candidats.
//
...
/** Ce que la vignette sait d'un candidat, transmis au panneau de détail. */
export type TrombinoscopeCandidate = Pick<
  OrganigrammeCandidate,
  "id" | "firstName" | "lastName" | "hasPhoto" | "photoUpdatedAt"
>;
...
export default function Trombinoscope({
  ...
  /** Clic sur une vignette : ouvre le panneau de détail (sinon, la fiche). */
  onOpen?: (c: TrombinoscopeCandidate) => void;
  selectedId?: string | null;
  /** Incrémenté par la page après une suppression / modification. */
  refreshKey?: number;
}) {
  const { user } = useAuth();
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className="text-sm text-gray-500">
          Les visages du recrutement, et vos coups de cœur. Cliquez sur une
          photo pour ouvrir la fiche.
        </p>
  ...
  return (
    ...
    >
      <a
        ...
        onClick={(e) => {
          // Clic simple : panneau de détail sur la même page. Cmd/Ctrl-clic
          // ou clic molette : la fiche complète dans un nouvel onglet.
