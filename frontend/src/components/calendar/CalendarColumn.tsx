...
export const CalendarColumn = ({ date, events, onEventClick, isMember, onTimeSlotClick, variant = 'time-grid' }: CalendarColumnProps) => {
        ...
        return (
            <div className="flex-1 min-w-[200px] border-r border-gray-100 last:border-r-0 flex flex-col">
                <div className="text-center p-3 border-b border-gray-100 bg-white sticky top-0 z-10">
                    <div className="text-sm font-medium text-gray-500 uppercase">{format(date, 'EEEE', { locale: fr })}</div>
                ...
                <div className="flex-1 bg-gray-50/10 p-2 space-y-2 overflow-y-auto">
                    {dayEvents.length === 0 && <div className="text-xs text-gray-400 text-center py-4">Aucun événement</div>}
                    {dayEvents.map((event) => (
                        <button
                            key={event.id}
                            ...
                            className={cn(
                                ...
                                event.isDeadline ? "bg-red-50 border-red-300 text-red-900 font-bold" :
                                event.isSlot ? "bg-purple-50 border-purple-300 text-purple-900" :
                                event.relatedEpreuveId ? "bg-blue-50 border-blue-200 text-blue-900" : "bg-white border-gray-200 text-gray-900"
                            )}
                        >
    ...
    const topDisabledHeight = dayConfig.isOpen ? (dayConfig.start - dayStart) * HOUR_HEIGHT : (dayEnd - dayStart) * HOUR_HEIGHT;
    const bottomDisabledTop = dayConfig.isOpen ? (dayConfig.end - dayStart) * HOUR_HEIGHT : 0;
    const bottomDisabledHeight = dayConfig.isOpen ? (dayEnd - dayConfig.end) * HOUR_HEIGHT : 0;
        ...
        <div className="flex-1 min-w-[200px] border-r border-gray-100 last:border-r-0 flex flex-col">
            ...
            <div
                className={cn(
                    ...
                    dayConfig.isOpen ? "bg-white cursor-pointer hover:bg-gray-50" : "bg-gray-100 cursor-not-allowed"
                )}
            ...
            >
                    ...
                    return (
                        <div
                            ...
                            className={cn(
                                ...
                                isFullHour ? "border-b-2" : "border-b border-dashed opacity-50"
                            )}
                ...
                {dayConfig.isOpen && topDisabledHeight > 0 && (
                    <div className="absolute top-0 left-0 right-0 bg-gray-100/50 border-b border-gray-200" style={{ height: topDisabledHeight }} />
                )}
                {dayConfig.isOpen && bottomDisabledHeight > 0 && (
                    <div className="absolute left-0 right-0 bg-gray-100/50 border-t border-gray-200" style={{ top: bottomDisabledTop, height: bottomDisabledHeight }} />
                )}

                {/* Events Rendering */}
                {dayConfig.isOpen && dayEvents.map((event) => {
